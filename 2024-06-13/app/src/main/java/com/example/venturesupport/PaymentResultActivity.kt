...
import com.example.venturesupport.RetrofitService.paymentService
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getUserInfo(accessToken,this@PaymentResultActivity){
            if(isSuccess(impResponse)){
                val payment = com.example.venturesupport.Payment(generateRandomLongInRange(0,1000000).toInt(),it!!,"test")
                val sendDataPayment = RetrofitService.paymentService.createPayment(payment)
                sendDataPayment.enqueue(object : Callback<com.example.venturesupport.Payment>{
                    override fun onResponse(
                        p0: Call<com.example.venturesupport.Payment>,
                        p1: Response<com.example.venturesupport.Payment>
                    ) {
                        // 응답 처리
                    }
        ...
        // 결과 메시지 텍스트 색상 설정
        resultMessage.setTextColor(ContextCompat.getColor(this, color))
    }
