...
public class VehicleInventoryController {
    ...
    @GetMapping("/users/{id}")
    public ResponseEntity<List<VehicleInventory>> getVehicleInventoriesByUserId(@PathVariable Integer userId) {
        List<VehicleInventory> vehicleInventories = vehicleInventoryService.getVehicleInventoriesByUserId(userId);
    ...
    public ResponseEntity<List<VehicleInventory>> getVehicleInventoriesByProductId(@PathVariable Integer productId) {
        List<VehicleInventory> vehicleInventories = vehicleInventoryService.getVehicleInventoriesByProductId(productId);
        if (!vehicleInventories.isEmpty()) {
            return new ResponseEntity<>(vehicleInventories, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
